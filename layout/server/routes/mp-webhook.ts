...
function signN8nPayload(params: { secret: string; template: string }) {
  const hex = crypto.createHmac("sha256", params.secret).update(params.template).digest("hex");
  return `sha256=${hex}`;
}

async function notifyN8nOrderPaid(params: { order: any; payment: any }) {
  if (!env.n8nPaidWebhookUrl) return;

  const address = typeof params.order?.shipping_address_json === "string"
  ...
  if (env.n8nPaidWebhookSecret) {
    ...
    const template = `ts:${timestampMs};order_id:${String(params.order.id)};payment_id:${String(params.payment?.id ?? "")};total_cents:${String(params.order.total_cents)};paid_at:${String(params.order.paid_at ?? "")};`;
    headers["x-taizacare-signature"] = signN8nPayload({ secret: env.n8nPaidWebhookSecret, template });
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[n8n] paid webhook failed", res.status, text.slice(0, 300));
    }
  } finally {
...
export const handleMercadoPagoWebhook: RequestHandler = async (req, res) => {
  try {
    ...
    if (status === "approved" && !wasPaid) {
      ...
      void notifyN8nOrderPaid({ order: orderPaid, payment }).catch((err) => {
